...
def main() -> int:
    ...
    if PLUGIN_DIR.exists():
        ...
        try:
            ...
            check(
                "allowlist non-empty",
                bool(cfg.get("allowed_targets")),
                f"{len(cfg.get('allowed_targets') or [])} rule(s)",
            )
            check(
                "confirm gate",
                bool(cfg.get("require_authorized_flag", True)),
        ...
        check(
            "plugin import (no strix needed)",
            True,
            f"plugin dir not present here ({PLUGIN_DIR}) — not a repo checkout",
            level=WARN,
        )
        cfg: dict = {}
    ...
    if wpy and not Path(wpy).exists() and shutil.which(wpy) is None:
        ...
        check(
            "worker strix installed",
            imp.returncode == 0,
            imp.stdout.strip() or imp.stderr.strip().splitlines()[-1],
        )
        if imp.returncode == 0:
            docker_ok = subprocess.run(
                [
                    wpy,
                    "-c",
                    "import docker; docker.from_env().ping(); print('docker ping OK')"
            ...
            check(
                "worker docker reachable",
                docker_ok.returncode == 0,
        ...
        check(
            "worker python",
            False,
            "not configured — set worker_python in strix.yaml or STRIX_WORKER_PYTHON",
        )
    ...
    if (deployed / "plugin.yaml").exists():
        ...
        check(
            "hermes plugin deployed",
            True,
            f"not present at {deployed} (this host may be worker-only)",
            level=WARN,
        )
