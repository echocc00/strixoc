...
| **0.4.0** | 运维自动化:systemd 模板 + retention + 告警 | ✅ 已发布 2026-08-16 | - |
| **0.5.0** | 安全强化:authz URL 规范化 + 审计加固 | ✅ 已发布 2026-08-16 | - |
| **0.6.0** | 全端口授权:规则 `:*` + allowed_ports(用户需求:整环境全端口扫描) | ✅ 已发布 2026-08-16 | - |
...
- [x] NAS 重装场景:只跑 `install_service.sh` 即恢复服务(代码/模板侧完成,NAS 实机验证待部署窗口)
- [x] prune dry-run 输出与实际删除一致;index.jsonl 保留摘要(单测覆盖)
- [x] 审计文件超过 10MB 自动轮转,旧审计可查(单测覆盖轮转与有界保留)
- [x] kill worker 进程,飞书 ≤2min 收到失败通知(看门狗 30s 轮询 + 90s 超时;单测覆盖检测矩阵,NAS 实测待部署窗口)
...
- [x] 攻击用例集全绿,每条用例注释攻击向量与 CVE/OWASP 对应(test_authz_url.py,10 类向量)
- [x] `make security-review`(根 repo bandit)无新增告警(新增代码 0 条;存量 12 条为既有模式)
- [x] 手工渗透演练:绕过尝试全部 deny 且留痕(用例集覆盖 = 自动化演练;细分 reason 落审计)
